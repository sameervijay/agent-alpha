"""
Technology Publications Domain Agent — covers process nodes, EUV, chiplets, HBM.
"""
...
SYSTEM_PROMPT = """You are a semiconductor technology expert who reads industry publications and research.
Your domain expertise covers:
- Process node transitions (3nm, 2nm, GAA transistors)
- EUV lithography (High-NA EUV, throughput, pellicle development)
- Advanced packaging (CoWoS, InFO, chiplets, 3D stacking)
- HBM (High Bandwidth Memory) technology and capacity
- AI accelerator architectures (GPU, TPU, custom ASICs)
- Design tools and IP (EDA tools, standard cell libraries)
- Yield improvements and manufacturing learning curves

Companies you analyze: NVIDIA (NVDA), TSMC (TSM), ASML (ASML), Cadence (CDNS), CoreWeave (CRWV)

When analyzing events, think about:
1. What is the technology change and its timeline?
2. How does this affect manufacturing costs (yield → gross margin)?
3. Does this enable new products or higher ASPs?
4. How does it shift competitive dynamics in the value chain?
...
class TechPublicationsAgent(BaseAgent):
    def __init__(self):
        super().__init__(
            name="tech_publications_agent",
            role_description="Semiconductor Technology Expert",
            system_prompt=SYSTEM_PROMPT,
        )

    def detect_events(self, news_input: str) -> list:
        prompt = f"""Analyze the following news/scenario for semiconductor technology events.
...
Only include events relevant to semiconductor technology."""
    ...
    def build_causal_links(self, event: Event) -> list:
        prompt = f"""Given this technology event, trace the causal chain to financial impacts.
...
- intermediate_step: technology mechanism
...
- reasoning: justification"""
    ...
    def debate_position(self, event: Event, metric: str, company: str,
        ...
        prompt = f"""You are debating the impact of an event on a financial metric from a technology perspective.
...
- confidence: float 0.0 to 1.0
- reasoning: 3-5 sentence justification from technology perspective
